    ...
    commitMutation({
      ...
      .then(result => {
        ...
        if (committedOnBranch !== path.split('/')[0] || view === 'new') {
          const newPath = `${committedOnBranch}/${path
            .split('/')
            .slice(1)
            .join('/')}`
          setTimeout(() => {
          ...
          <ProcessToolbar>
            {uncommittedChanges &&
              <div {...css(styles.uncommittedChanges)}>
                ...
                <Button onClick={this.revertHandler} style={styles.button}>
                  Revert
                </Button>
                <Button onClick={this.commitHandler} style={styles.button}>
                  Commit
                </Button>
              </div>}
            {!uncommittedChanges &&
              <div {...css(styles.uncommittedChanges)}>
                <Label>All changes committed</Label>
              </div>}
            <Label>Checklist</Label>
            <Checklist disabled={!!uncommittedChanges} />
            <Label>History</Label>
            <FileCommits {...this.props} />
            <Label>Who's working on this?</Label>
            <UncommittedChanges {...this.props} />
          </ProcessToolbar>
        </div>
      )
    } else if (text) {
