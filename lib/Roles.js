...
const getRootQueryName = s =>
  s.definitions[0].selectionSet.selections[0].name.value

const createRoleMiddleware = (role, exceptions) => {
  return (req, res, next) => {
    const rootQuery = getRootQueryName(parse(req.body.query))
    if (exceptions.indexOf(rootQuery) > -1) {
      return next()
    }
    try {
      ensureUserHasRole(req.user, role)
    } catch (e) {
      res.status(401)
