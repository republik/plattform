...
const createBlockButton = get => type => {
  class BlockButton extends Component {
    ...
    render () {
      ...
      const isDisabled = this.isDisabled()
      const onMouseDown = !isDisabled || isActive
        ? e => this.clickHandler(e)
        : e => e.preventDefault()
      const Button = get(
...
const createStringButton = get => (prefix, suffix) => {
  class BlockButton extends Component {
    isDisabled () {
      const { state } = this.props
      return state.isBlurred || (suffix && state.isEmpty)
    }

    clickHandler (e) {
      e.preventDefault()
      let { state, onChange } = this.props
      const derivedSuffix = !suffix
        ? !state.isEmpty
          ? prefix
          : ''
        : suffix
      console.log(derivedSuffix)
      state = state
        .transform()
        .wrapText(prefix, derivedSuffix)
        .apply()
...
export default get => ({
  ...
  createStringButton: createStringButton(get)
})
