...
export default withT(({ t, editor, node, onInputChange }) => {
  const audioCoverAnchors = [null, 'middle'].map(value => ({
    value, text: t(`metaData/audio/cover/anchor/${value}`)
  }))
  ...
  return <Fragment>
    ...
    <MetaForm data={audioSourceData} onInputChange={onInputChange} getWidth={() => ''} black />
    <UIForm getWidth={() => '25%'}>
      <Dropdown
        black
        ...
        onChange={({ value }) => onChange('audioCover')(value && {
          anchor: value,
          color: (audioCover && audioCover.color) || '#fff',
          backgroundColor: (audioCover && audioCover.backgroundColor) || 'rgba(255,255,255,0.3)'
        })}
      />
      {audioCover && <Field
        black
