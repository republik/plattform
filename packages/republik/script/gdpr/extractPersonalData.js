...
const { findPublished } = require('@orbiting/backend-modules-search/lib/Documents')
...
const getMetas = (repoIds, { elastic }) => Promise.map(repoIds, async repoId => {
...
Promise.props({ pgdb: PgDb.connect(), elastic: Elasticsearch.connect() }).then(async connections => {
  ...
  // -granted
  const accessGrantsGranted = await pgdb.public.accessGrants.find({ granterUserId: user.id })

  if (accessGrantsGranted.length > 0) {
    const accessCampaigns = await pgdb.public.accessCampaigns
      .find({ id: accessGrantsGranted.map(({ accessCampaignId }) => accessCampaignId) })
  ...
  // -claimed
  const accessGrantsClaimed = await pgdb.public.accessGrants
    .find({ or: [{ recipientUserId: user.id }, { email: user.email }] })

  if (accessGrantsClaimed.length > 0) {
    const accessCampaigns = await pgdb.public.accessCampaigns
      .find({ id: accessGrantsClaimed.map(({ accessCampaignId }) => accessCampaignId) })
  ...
  if (collectionDocumentItems.length > 0) {
    ...
    const metas = await getMetas(collectionDocumentItems.map(({ repoId }) => repoId), { elastic })
    ...
    await save(
      ...
      collectionDocumentItems.map(collectionDocumentItem => {
        ...
        const meta = metas.find(meta => meta.repoId === collectionDocumentItem.repoId)
  ...
  if (commentsPosted.length > 0) {
    const discussions = await pgdb.public.discussions
      .find({ id: commentsPosted.map(({ discussionId }) => discussionId) })
  ...
  if (commentsVoted.length > 0) {
    const discussions = await pgdb.public.discussions
      .find({ id: commentsVoted.map(({ discussionId }) => discussionId) })
  ...
  if (commentsReported.length > 0) {
    const discussions = await pgdb.public.discussions
      .find({ id: commentsReported.map(({ discussionId }) => discussionId) })
  ...
  await save(
    ...
    consents.map(consent => pick(consent, ['policy', 'ip', 'record', 'createdAt']))
  )
  ...
  await save(
    ...
    'credentials',
    'erfasste Rollen',
    credentials.map(credential => pick(credential, ['description', 'isListed', 'verified', 'createdAt', 'updatedAt']))
  )
  ...
  await save(
    ...
    'devices',
    'angemeldete Geräte',
    devices.map(device => pick(device, ['information', 'createdAt', 'updatedAt']))
  )
  ...
  await save(
    ...
    eventLog.map(record => pick(record, ['createdAt', 'schemaName', 'tableName', 'action']))
  )
  ...
  if (memberships.length > 0) {
    ...
    await save(
      ...
      membershipCancellations.map(
        cancellation => pick(cancellation, ['createdAt', 'revokedAt', 'category', 'reason'])
      )
    )
    ...
    await save(
      ...
      chargeAttempts.map(attempt => pick(attempt, ['createdAt', 'total', 'status']))
    )
  }
  ...
  if (pledges.length > 0) {
    ...
    await save(
      ...
      pledgeOptions.map(pledgeOption => {
        ...
        const pledge = pledges.find(pledge => pledge.id === pledgeOption.pledgeId)
        const package_ = packages.find(package_ => package_.id === pledge.packageId)

        return {
          reward__type: (membershipReward && membershipReward.rewardType) || (goodieReward && goodieReward.rewardType),
          reward__name: (membershipReward && membershipReward.name) || (goodieReward && goodieReward.name),
          ...pick(pledgeOption, ['createdAt', 'price', 'amount', 'periods', 'total']),
  ...
  if (previewRequests.length > 0) {
    await save(
      ...
      previewRequests.map(request => pick(request, ['createdAt', 'scheduledAt', 'expiredAt', 'followupAt']))
    )
  }
  ...
  await save(
    ...
    tokens.map(token => pick(token, ['createdAt', 'context', 'type', 'email', 'expiresAt', 'expireAction']))
  )
  ...
  await save(
    ...
    subscriptions.map(subscription => pick(subscription, ['createdAt', 'active', 'objectType']))
  )
  ...
  await save(
    ...
    notifications.map(notification => pick(notification, ['createdAt', 'eventObjectType', 'channels', 'readAt']))
  )
