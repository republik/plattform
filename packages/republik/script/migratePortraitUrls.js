  ...
  .then(async (pgdb) => {
    ...
    await Promise.map(
      ...
      async (user) => {
        ...
        portraitUrl = portraitUrl.replace(
          'https://assets.republik.ch/',
          'https://republik-assets.s3.eu-central-1.amazonaws.com/',
        )
        ...
        const newPortraitUrl = await Portrait.upload(portrait, dry).catch(
          (error) => {
