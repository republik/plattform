...
import {
  ...
  totalChf,
  colors,
} from '../web/RepublikShareholder.data'
...
export default () => (
  <Table>
    ...
    <tbody>
      {groupped.children.map((group, i) => {
        ...
        const elements = [
          <tr key={i}>
            ...
            <th style={{ ...style, ...styles.num }}>
              {countFormat(group.value)}
            </th>
            <th style={{ ...style, ...styles.num }}>
              {percentFormat(group.value / total)}
            </th>
            <th style={{ ...style, ...styles.num }}>
              {percentFormat(
          ...
          group.children.forEach((entity, i) => {
            elements.push(
              <tr key={`entity${i}`}>
                ...
                <td style={{ ...styles.trTdTh, ...styles.num }}>
                  {countFormat(entity.value)}
                </td>
                <td style={{ ...styles.trTdTh, ...styles.num }}>
                  {percentFormat(entity.value / total)}
                </td>
                <td style={{ ...styles.trTdTh, ...styles.num }}>
                  {percentFormat(
      ...
      <tr>
        <th style={{ ...styles.groupTrTdTh, ...styles.groupTrLastTdTh }}>
          Total
        </th>
        <th
          style={{
            ...styles.groupTrTdTh,
            ...styles.groupTrLastTdTh,
            ...styles.num,
          }}
        >
          {countFormat(total)}
