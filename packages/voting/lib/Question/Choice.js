...
const validate = (value, question, { t }) => {
  ...
  if (value.length === 0) {
    ...
    if (
      question.typePayload.cardinality > 0 &&
      value.length > question.typePayload.cardinality
    ) {
      throw new Error(
        t('api/questionnaire/answer/Choice/tooMany', {
          max: question.typePayload.cardinality,
        }),
      )
    }
    for (let v of value) {
      if (!question.typePayload.options.find((ov) => ov.value === v)) {
        throw new Error(
          t('api/questionnaire/answer/Choice/value/404', { value: v }),
        )
      }
    }
  }
