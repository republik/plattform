...
PgDb.connect()
  .then(async (pgdb) => {
    const newElection = await pgdb.public.elections.findOne({
      id: config.newElectionId,
    })
    if (!newElection) {
  ...
  `,
    ...
    if (candidacies.length !== config.candidacyIds.length) {
      throw new Error(
        'some of the requested candidates could not be found',
        candidacies,
        config.candidacyIds,
      )
    }
    ...
    await Promise.all(
      candidacies.map(async (c) => {
        ...
        if (existingCandidacy) {
          console.log(
            `skipping ${c.user.firstName} ${c.user.lastName} exists as ${existingCandidacy.id}`,
          )
          return
        }
