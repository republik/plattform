...
describe('submitPledge', () => {
  test('default pledge with ABO package', async () => {
    ...
    const membership2 = await pgDatabase().public.memberships.count({
      pledgeId: result2.data.submitPledge.pledgeId,
    })
...
describe('updatePayment', () => {
  ...
  test('WAITING -> CANCELLED', async () => {
    ...
    const { paymentId } = await pgDatabase().public.pledgePayments.findFirst({
      pledgeId,
    })
...
describe('claimMembership', () => {
  ...
  test('Claim a membership with package isAutoActivateUserMembership disabled', async () => {
    const { pledgeId } = await prepareNewPledge({
      templateId: '00000000-0000-0000-0008-000000000006',
    })
    await payPledge({
      pledgeId,
      method: PAYMENT_METHODS.PAYMENTSLIP,
    ...
    const membership = await pgDatabase().public.memberships.findFirst({
      pledgeId,
    })
    expect(membership).toBeTruthy()
...
describe('reclaimPledge', () => {
  ...
  test('Claim a pledge of a verified user', async () => {
    const { pledgeId, apolloFetch } = await prepareNewPledge({
      user: Users.Member,
    })
...
describe('cancelPledge', () => {
  ...
  test('Waiting ABO with PAYMENTSLIP', async () => {
    ...
    expect(
      moment(
        result.data.cancelPledge.memberships[0].periods[0].endDate,
      ).isBefore(afterEndDate),
    ).toBeTruthy()
