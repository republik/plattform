exports.minTotal = (pledgeOptions, packageOptions) =>
  Math.max(
    pledgeOptions.reduce((price, plo) => {
      const pko = packageOptions.find((pko) => pko.id === plo.templateId)
      const periods =
        plo.periods || (pko.reward && pko.reward.defaultPeriods) || 1
...
exports.regularTotal = (pledgeOptions, packageOptions) =>
  Math.max(
    pledgeOptions.reduce((price, plo) => {
      const pko = packageOptions.find((pko) => pko.id === plo.templateId)
      const periods =
