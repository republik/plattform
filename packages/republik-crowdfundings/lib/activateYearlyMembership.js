...
module.exports = async (memberships, pgdb, dryRun) => {
  ...
  const packagesIndex = (await pgdb.public.packages.findAll()).reduce(
    (index, p) => {
      index[p.id] = p
      return index
    },
  ...
  const yearlyMemberships = memberships.filter(
    (m) => membershipTypeIndex[m.membershipTypeId].name !== 'MONTHLY_ABO',
  )
  ...
  const unusedMemberships = yearlyMemberships.filter(
    (m) => !membershipPeriods.find((p) => p.membershipId === m.id),
  )
  ...
  const inactiveMemberships = unusedMemberships.filter((m) => {
    ...
    return (
      !['ABO_GIVE', 'ABO_GIVE_MONTHS'].includes(
        packagesIndex[pledge.packageId].name,
      ) || !m.voucherCode
    )
  })
  ...
  inactiveMemberships.sort((a, b) =>
    ascending(a.sequenceNumber, b.sequenceNumber),
  )

  const electedMembership =
    inactiveMemberships.find((m) => m.voucherCode === null) ||
    inactiveMemberships.find((m) => m.reducedPrice) ||
    inactiveMemberships.find(
      (m) => membershipTypeIndex[m.membershipTypeId].name === 'BENEFACTOR_ABO',
    ) ||
