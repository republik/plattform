...
const argv = yargs
  ...
  .option('name', { alias: ['n'] })
  .usage(
    'Usage: $0 --sqldir <folder to store sqls (rel repo root)> --name <name of migration>',
  )
...
// write sql files
fs.writeFileSync(upFileAbs, '-- migrate up here: CREATE TABLE...')
fs.writeFileSync(downFileAbs, '-- migrate down here: DROP TABLE...')
