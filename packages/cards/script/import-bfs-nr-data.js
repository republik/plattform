   ...
   */
    ...
    await Promise.each(kandidierende, async (k, index) => {
      ...
      */
      ...
      const matchedCards = cards
        ...
        .filter(
          (c, index, array) =>
            array.length === 1 || c.payload.yearOfBirth === k.geburtsjahr,
        )
    ...
    `)
      ...
      const content = [
        ...
        dataStats.listNationalCouncilCantons &&
          `${
            dataStats.countNationalCouncilCantons
          } Kantone: ${dataStats.listNationalCouncilCantons.join(', ')}`,
      ]
        .filter(Boolean)
        .join('\n')
