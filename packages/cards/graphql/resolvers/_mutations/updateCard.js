...
module.exports = async (_, { id, portrait, statement, payload }, context) => {
  ...
  try {
    ...
    const updatedPayload = {
      ...
      ...(payload.campaignBudget !== undefined && {
        campaignBudget:
          (payload.campaignBudget && Number(payload.campaignBudget)) || null,
      }),
      ...
      ...(payload.vestedInterests !== undefined && {
        vestedInterests: payload.vestedInterests.map(
    ...
    try {
      await publish(
        ...
        [
          ...
          user._raw.portraitUrl &&
            `<${user._raw.portraitUrl}|${portrait ? 'neue ' : ''}Portrait-URL>`,
        ]
          .filter(Boolean)
          .join('\n'),
      )
