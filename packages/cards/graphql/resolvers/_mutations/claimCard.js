...
module.exports = async (
  ...
  try {
    const card = await transaction.public.cards.findOne({
      id,
      userId: tokenUser.id,
    })
    if (!card) {
    ...
    if (user.id !== tokenUser.id) {
      ...
      await transaction.public.users.update(
        { id: user.id },
        {
          ...
          portraitUrl:
            portraitUrl || tokenUser._raw.portraitUrl || user._raw.portraitUrl,
          facebookId: user._raw.facebookId || tokenUser._raw.facebookId,
          twitterHandle:
            user._raw.twitterHandle || tokenUser._raw.twitterHandle,
      ...
      if (
        user._raw.username &&
        user._raw.username !== tokenUser._raw.username
      ) {
        try {
    ...
    try {
      ...
      await publish(
        SLACK_CHANNEL_FEED,
        [
          `:fire: *claimCard* Card von *${[
            card.payload.meta.firstName,
            card.payload.meta.lastName,
          ].join(' ')}*`,
          `_Republik Card ID: ${card.id}, Smartvote ID: ${card.payload.meta.userId}_`,
          `durch *<${ADMIN_FRONTEND_BASE_URL}/users/${owner.id}|${[
            owner.firstName,
            owner.lastName,
          ].join(' ')}>*`,
          statement &&
          ...
          owner.portraitUrl &&
            `<${owner.portraitUrl}|${portrait ? 'neue ' : ''}Portrait-URL>`,
        ]
          .filter(Boolean)
          .join('\n'),
      )
    ...
    await publish(
      ...
      [
        ...
        e.message,
        user &&
          `durch *<${ADMIN_FRONTEND_BASE_URL}/users/${user.id}|${[
            user._raw.firstName,
            user._raw.lastName,
          ].join(' ')}>*`,
