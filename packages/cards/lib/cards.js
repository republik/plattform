...
const buildDeck = (cards, seed, focus = [], smartspider = []) => {
    ...
    .map((card) => {
      ...
      Array.from(Array(8).keys()).forEach((_, index) => {
        if (
          ...
          distances.push(
            Math.abs(smartvoteCleavage[index] - smartspider[index]),
          )
      ...
      return {
        ...card,
        _distance: +(
          distances.reduce((a, c) => a + c, 0) / smartspiderCount
        ).toFixed(2),
      }
    })
    .map((card) => {
      const _mandates =
        0 +
        (card.payload.nationalCouncil && card.payload.nationalCouncil.incumbent
          ? 1
          : 0) +
        (card.payload.councilOfStates && card.payload.councilOfStates.incumbent
          ? 1
          : 0)
  ...
  const focusCards = deck
    .filter((c) => focus.includes(c.id))
    .sort((a, b) => ascending(focus.indexOf(a.id), focus.indexOf(b.id)))

  const smartspiderRelevantCards = deck
    .filter((c) => c._distance < 100)
    .sort((a, b) => ascending(a._distance, b._distance))

  const shuffledCards = shuffleSeed
    .shuffle(deck, seed)
    .sort((a, b) =>
      descending(
        (a.payload.nationalCouncil.votes || 0) +
          (a.payload.councilOfStates.votes || 0),
        (b.payload.nationalCouncil.votes || 0) +
          (b.payload.councilOfStates.votes || 0),
      ),
    )
    .sort((a, b) =>
      descending(
        a.payload.nationalCouncil.elected ||
          a.payload.councilOfStates.elected ||
          0,
        b.payload.nationalCouncil.elected ||
          b.payload.councilOfStates.elected ||
          0,
      ),
    )

  return removeDuplicates(
    [...focusCards, ...smartspiderRelevantCards, ...shuffledCards],
    'id',
  )
}
...
const filterCards = async (cards, { filters = {} }, context) => {
  ...
  if (filteredCards.length > 0 && filters.candidacies) {
    filteredCards = filteredCards.filter((card) => {
      return filters.candidacies.every(
        (election) =>
          card.payload &&
  ...
  if (filteredCards.length > 0 && filters.elected) {
    filteredCards = filteredCards.filter((card) => {
      return ['nationalCouncil', 'councilOfStates'].some(
        (election) =>
          card.payload &&
          card.payload[election] &&
          card.payload[election].elected,
  ...
  if (filteredCards.length > 0 && filters.subscribedByMe) {
    if (context.user) {
      ...
      filteredCards = filteredCards.filter((card) =>
        subscribedUserIds.includes(card.userId),
      )
    } else {
      filteredCards = []
    }
...
const upsertStatement = async (_card, transaction) => {
  ...
  const cardGroup = await transaction.public.cardGroups.findOne({
    id: card.cardGroupId,
  })
  const discussion = await transaction.public.discussions.findOne({
    id: cardGroup.discussionId,
  })
  const now = new Date()
