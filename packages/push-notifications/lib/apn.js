...
if (!APN_KEY || !APN_KEY_ID || !APN_TEAM_ID || !APN_BUNDLE_ID) {
  console.warn(
    'missing env APN_*, sending push notifications via apn will not work',
  )
} else {
  const options = {
    token: {
...
const publish = async (args, pgdb) => {
  ...
  if (tokens.length > 0) {
    const message = new apn.Notification({
      ...
      ...(ttl // A UNIX epoch date expressed in seconds (UTC).
        ? {
            expiry: parseInt(
              new Date(
                now.getTime() + now.getTimezoneOffset() + ttl,
              ).getTime() / 1000,
            ),
          }
        : {}),
    ...
    const results = await Promise.all(
      tokens.map((token) => provider.send(message, token)),
    )
    debug(
      'APN: #recipients %d, message: %O, results: %O',
      tokens.length,
      message,
      results,
    )
