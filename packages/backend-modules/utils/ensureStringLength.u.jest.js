...
describe('ensureStringLength', () => {
  ...
  test(`in range`, () => {
    expect(() =>
      ensureStringLength(testString, { min: 1, max: 4 }),
    ).not.toThrow()
    expect(() =>
      ensureStringLength(testString, { min: -1, max: 40 }),
    ).not.toThrow()
  })

  test(`float range`, () => {
    expect(() =>
      ensureStringLength(testString, { min: 1.6, max: 4.1 }),
    ).not.toThrow()
    expect(() =>
      ensureStringLength(testString, { min: 1.6, max: 3.9 }),
    ).toThrow()
  })
  ...
  test('custom error', () => {
    expect(() =>
      ensureStringLength(testString, {
        min: 1,
        max: 3,
        error: 'CUSTOM_ERROR',
      }).toThrow('CUSTOM_ERROR'),
    )
  })
})
