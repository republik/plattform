  ...
  try {
    ...
    if (existingDevice) {
      ...
      if (existingDevice.userId !== me.id) {
        debug(
          'changing ownership of existing device. oldUser: %s newUser: %s',
      ...
      device = await transaction.public.devices.updateAndGetOne(
      ...
      if (!information) {
        console.error('information required for device insert', {
          token,
          information,
        })
