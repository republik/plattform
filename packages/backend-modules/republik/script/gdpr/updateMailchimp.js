...
PgDb.connect()
  ...
  `)
    ...
    await Promise.all(
      mailchimpEmails.map(async (email) => {
        // mailchimpEmails.forEach( email => {
        const user = users.find(
          (u) => u.email.toLowerCase() === email.toLowerCase(),
        )
        const segmentData = await getSegmentDataForUser({ user, pgdb, getNewsletterInterests: (email) => {}, })
    ...
    if (diffInput) {
      ...
      console.log(
        `#diff emailsWithSubscribeUrl <> diffInput: ${
          emailsWithSubscribeUrl.length - diffInput.length
        }`,
      )
      // console.log(util.inspect(missingInDiffInput, {depth: null}))
      fs.writeFileSync(
        __dirname.concat('/diff.csv'),
        missingInDiffInput.join('\n'),
      )
    }
