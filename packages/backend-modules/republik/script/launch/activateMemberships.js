...
PgDb.connect()
  .then(async (pgdb) => {
    ...
    `)
      ...
      for (let user of usersWithMemberships) {
        ...
        if (user.memberships.length === 1) {
          ...
          if (membershipsWithoutVoucherCode.length > 1) {
            console.warn(
              'multiple memberships without voucherCode!!!!!!!!',
              user.memberships,
            )
          }

          const reducedPriceMembership = user.memberships.find(
            (m) => m.reducedPrice,
          )
          ...
          const benefactorMemberships = user.memberships
            .filter(
              (membership) =>
                membership.membershipTypeId === benefactorMembershipType.id,
            )
            .sort((a, b) => ascending(a.sequenceNumber, b.sequenceNumber))
          ...
          electedMembership =
            membershipsWithoutVoucherCode[0] ||
      ...
      console.log(
        'memberships !active:',
        await transaction.public.memberships.count({ active: false }),
        '\n',
      )
