...
console.log(
  `running createStripeCustomers.js${testMode ? ' (in TEST mode)' : ''}...`,
)
PgDb.connect()
  .then(async (pgdb) => {
  ...
  `)
    ...
    for (let user of users) {
      ...
      try {
        ...
        if (
          !(await transaction.public.stripeCustomers.findFirst({
            userId: user.id,
          }))
        ) {
        ...
        for (let source of user.sources) {
          try {
            ...
            console.log(
              '\tfailed to add source (moving to PAYMENTSLIP):',
              e2.message,
              source.pspId,
            )
    ...
    console.log('\nResults:')
    console.log(
      `error for (${errorUserEmails.length}) emails:`,
      errorUserEmails.length ? errorUserEmails.join(', ') : 'none',
    )
    console.log(
      `success for (${doneUserEmails.length}) emails:`,
      doneUserEmails.length ? doneUserEmails.join(', ') : 'none',
    )
    console.log(
      `investigate (${investigateUserEmails.length}) emails:`,
      investigateUserEmails.length ? investigateUserEmails.join(', ') : 'none',
    )
    console.log(
      'stripeCustomers total:',
      await pgdb.queryOneField(
        'SELECT count(distinct("userId")) as count from "stripeCustomers"',
      ),
    )
