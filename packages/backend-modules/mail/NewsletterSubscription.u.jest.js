...
test('NewsletterSubscription -> correct flags', async () => {
  ...
  const sub1 = NewsletterSubscription.buildSubscription(
    'user_x',
    INTEREST_ID_MEMBER_RESTRICTED,
    true,
    ['member'],
  )
  expect({
  ...
  const sub2 = NewsletterSubscription.buildSubscription(
    'user_x',
    INTEREST_ID_MEMBER_RESTRICTED,
    true,
    [],
  ...
  const sub3 = NewsletterSubscription.buildSubscription(
    'user_x',
    INTEREST_ID_PUBLIC,
    false,
    [],
...
test('NewsletterSubscription -> all interests', async () => {
  ...
  expect(NewsletterSubscription.allInterestConfigurations()).toEqual(
    interestConfiguration,
  )
})
