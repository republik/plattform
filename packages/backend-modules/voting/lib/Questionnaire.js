...
const getQuestions = async (questionnaire, args = {}, pgdb) => {
  const { orderFilter, includeHidden = false } = args
  if (questionnaire.result) {
    return questionnaire.result.questions
      ...
      .filter(
        (question) => !orderFilter || orderFilter.indexOf(question.order) > -1,
      )
  ...
  if (args.shuffle) {
    // +1 for weights > 0
    const getWeight = (question) =>
      question && question.metadata && question.metadata.weight + 1
    ...
    if (
      ...
      const sum = questions.reduce((agg, q) => agg + getWeight(q), 0)
    ...
    return shuffle(questions).slice(0, args.shuffle)
  }
  return questions
}
...
const getQuestionsWithResults = async (questionnaire, context) => {
  const { pgdb } = context
  return getQuestions(questionnaire, {}, pgdb).then((questions) =>
    Promise.all(
...
const getResult = async (questionnaire, context) => {
  ...
  const questions = await getQuestionsWithResults(
    questionnaireWithTurnout,
    context,
  )
  const now = new Date()
  return {
...
const updateResultIncrementally = async (
  questionnaireId,
  answer,
  transaction,
  context,
...
const refreshResult = async (questionnaireId, transaction, context) => {
  ...
  const result = await getResult(questionnaire, {
    ...context,
    pgdb: transaction,
  })
