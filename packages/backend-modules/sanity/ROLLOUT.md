...
1. `20260729120000-add-sanity-id-to-collection-document-items` — adds `collectionDocumentItems.sanityId` (nullable), drops `repoId`'s `NOT NULL`, adds the one-document-ref `CHECK` constraint `NOT VALID`.
2. `20260729120001-validate-sanity-id-check-collection-document-items` — validates that constraint. **Must run as its own migration, not bundled into the file above** — db-migrate wraps each migration file in one transaction, so validating in the same file would still hold the `ACCESS EXCLUSIVE` lock from the `ALTER TABLE` for the whole scan. Splitting them is what lets the scan run under the lighter `SHARE UPDATE EXCLUSIVE` lock instead, on a table (`collectionDocumentItems`) written on essentially every article view.
