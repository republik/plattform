...
Three new ones. **Order matters** — `next-reads-sanity-views` selects `collectionDocumentItems.sanityId` in a materialized view definition, so it must run after the migration that creates that column; filenames are timestamped to enforce this via db-migrate's run order, run in order as part of the deploy (not automatic):

1. `20260909120000-add-sanity-id-to-collection-document-items` — adds `collectionDocumentItems.sanityId` (nullable), drops `repoId`'s `NOT NULL`, adds a `CHECK (num_nonnulls("repoId", "sanityId") >= 1)` constraint `NOT VALID` (a row must have at least one of the two set — both may be set at once, see the dual-write note below). Written idempotently (`IF NOT EXISTS`/`DROP ... IF EXISTS` before re-adding) so it applies cleanly whether the table has none of this yet or already has an earlier, stricter version of the same constraint from before it was relaxed — no manual reconciliation needed on any environment, just a normal `migrate up`.
2. `20260909120001-validate-sanity-id-check-collection-document-items` — validates that constraint. **Must run as its own migration, not bundled into the file above** — db-migrate wraps each migration file in one transaction, so validating in the same file would still hold the `ACCESS EXCLUSIVE` lock from the `ALTER TABLE` for the whole scan. Splitting them is what lets the scan run under the lighter `SHARE UPDATE EXCLUSIVE` lock instead, on a table (`collectionDocumentItems`) written on essentially every article view.
3. `20260910120000-next-reads-sanity-views` — creates the `next_reads_sanity` schema/materialized views/`discussion_refs` table. Depends on migration 1 above (see "order matters").

**Incident note**: an earlier version of this doc had migration 1/2 timestamped `20260915...`, sorting *after* `next-reads-sanity-views` — a from-scratch `migrate up` (production) failed with `column "sanityId" does not exist` because the view-creation migration ran first and referenced a column that didn't exist yet. Staging didn't catch this because it already had the column from a since-replaced earlier version of the migration. Fixed by retimestamping to `20260909...`, sorting correctly before `20260910120000`. If these migrations are ever touched again, keep them earlier than `20260910120000-next-reads-sanity-views`.

If these haven't run when the app deploys, `SanityReadingPositionRefreshWorker`/`SanityNext7DaysFeedRefreshWorker`/`SanityNext20DaysCommentsFeedRefreshWorker` (scheduled unconditionally, see below) will fail their first cron job post-deploy against a nonexistent schema — not a crash, just a failed/retried queue job, but land the migrations with the deploy rather than after.
