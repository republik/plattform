...
PgDb.connect()
  .then(async (pgdb) => {
    ...
    try {
      const granter = await transaction.public.users.findOne({
        id: argv.granter,
      })
      ...
      while (grants.length < argv.amount && iteration < 5 * argv.amount) {
        ...
        const grant = {
          voucherCode: await transaction.queryOneField(
            `SELECT make_hrid('"accessGrants"'::regclass, 'voucherCode'::text, ${grantsLib.VOUCHER_CODE_LENGTH}::bigint)`,
          ),
        }
        ...
        if (argv.prefix) {
          grant.voucherCode =
            argv.prefix +
            grant.voucherCode.slice(
              argv.prefix.length,
              grantsLib.VOUCHER_CODE_LENGTH,
            )
        }
        ...
        if (
          ...
          !grants.find(
            ({ voucherCode }) => voucherCode === grant.voucherCode,
          ) &&
          ...
          (!argv.prefix ||
            (argv.prefix && // Lookup in database if argv.prefix is set
              (await transaction.public.accessGrants.count({
                voucherCode: grant.voucherCode,
              })) === 0))
        ) {
          grants.push(grant)
          debug('grant %s (%i/%i)', grant.voucherCode, grants.length, argv.amount)
        }
      }
      ...
      if (grants.length < argv.amount) {
        throw new Error(
          `Unable to generate ${argv.amount} unqiue voucher codes. Change parameters.`,
        )
      }
