...
const { GSHEETS_IMPORT, GSHEETS_EXPORT } = process.env

module.exports = (server, pgdb) => {
  if (!GSHEETS_IMPORT) {
    console.warn(
      'missing env GSHEETS_IMPORT, importing via the /gsheets/import endpoint will not work',
    )
  } else {
  ...
  if (!GSHEETS_EXPORT) {
    console.warn(
      'missing env GSHEETS_EXPORT, exporting via the /gsheets/export endpoint will not work',
    )
  } else {
