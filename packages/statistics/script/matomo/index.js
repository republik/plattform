...
PgDb.connect()
  .then(async (pgdb) => {
    ...
    const expressions = fields.map((field) => ({
      expression: `"${field.column_name}"`,
      outputName: field.column_name,
    }))
    ...
    SELECT
      ...
      ${expressions
        .map(
          ({ outputName }) =>
            `percentile_disc(0.5) WITHIN GROUP (ORDER BY "${outputName}") AS "${outputName}.p50"`,
        )
        .join(',\n')}
    ...
    if (queryResults.length > 0) {
      ...
      if (!dryRun && queryResults.length > 0) {
        ...
        if (hasRow) {
          ...
          await pgdb.public.statisticsIndexes.update(condition, {
            data,
            updatedAt: new Date(),
          })
        } else {
          debug('insert new index data for %o', condition)
          await pgdb.public.statisticsIndexes.insert({ ...condition, data })
        }
      } else {
