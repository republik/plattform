...
ConnectionContext.create('backends statistics publicationData').then(
  async (connections) => {
  ...
  */
    const {
      data: {
        documents: { nodes: documentsMeta },
      },
    } = require('../data/documents-20200509-1000.json')
    ...
    const documents = documentsMeta.map((d) => {
      ...
      return {
        ...
        linkedAuthors: d.users
          ...
          .join(', '),
        linkedAuthorComments: comments.filter(
          (c) =>
            c.discussionId === d.discussion &&
            d.users.map((u) => u.id).includes(c.userId),
        ).length,
      }
    })
