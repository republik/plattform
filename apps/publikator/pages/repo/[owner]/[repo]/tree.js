...
import EditorNav from '../../../../components/Edit/Nav'
...
const EditorPage = ({ router, commits, hasMore, fetchMore, t, data }) => {
  const [unsubscribe, setUnsubscribe] = useState(null)
  const { loading, error, repo } = data
  const repoId = getRepoIdFromQuery(router.query)

  useEffect(() => {
    if (!unsubscribe && data.repo) {
      const unsub = data.subscribeToMore({
