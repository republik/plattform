...
const resolvePledgeToPaymentHandler = (handler: any, pledge: Pledge) => () => {
  if (handler) {
...
const updatePaymentStatusHandler = (handler: any, pledge: Pledge) => () => {
...
const PaymentStatusButton = ({ pledge, ...props }: any) => {
  if (pledge.payments.length <= 0) {
    ...
    if (status === 'WAITING' || status === 'WAITING_FOR_REFUND') {
      const label = status === 'WAITING' ? 'Set to paid' : 'Set to refunded'
      return (
        <Button {...props}>
          {label}
        </Button>
...
const PledgeOverview = ({
  ...
  return (
    <div>
      ...
      <Button onClick={resolvePledgeToPaymentHandler(onResolvePledge, pledge)}>
        {' '}Resolve Pledge
      </Button>{' '}
      <PaymentStatusButton
        pledge={pledge}
        onClick={updatePaymentStatusHandler(onUpdatePaymentStatus, pledge)}
      />
    </div>
  )
}
